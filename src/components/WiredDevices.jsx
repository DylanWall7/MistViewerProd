...
export const WiredDevices = ({ clients = [], loading = false, siteId }) => {
  const [searchValue, setSearchValue] = useState("");
  const [sortDescriptor, setSortDescriptor] = useState({
    column: "last_hostname",
    direction: "ascending",
  });
  ...
  const sortedAndFilteredClients = useMemo(() => {
    ...
    if (searchValue) {
      ...
      items = items.filter((client) => {
        const ip = (client.last_ip || "").toLowerCase();
        const mac = normalizeMac(client.mac || "");
        const hostname = (client.last_hostname || "").toLowerCase();
    ...
    return (
      <div className="grid mt-40 px-4 place-content-center">
        <h1 className="tracking-widest text-gray-500 uppercase">
          No Site Selected | SELECT A SITE TO VIEW WIRED CLIENTS
        </h1>
      </div>
    );
  };
  ...
  const NoClientsMessage = () => {
    return (
      <div className="grid mt-40 px-4 place-content-center">
        <h1 className="tracking-widest text-gray-500 uppercase">
          No Wired Clients Found at Selected Site
        </h1>
      </div>
    );
  };
  ...
  if (loading) {
    return (
      <div className="grid mt-40 px-4 place-content-center">
        ...
        <p className="mt-4 text-gray-500">Loading wired clients...</p>
      </div>
    );
  }
      ...
      {sortedAndFilteredClients.length === 0 ? (
        ...
        <>
          <Table
            ...
            aria-label="Wired clients table"
            bottomContent={
              totalPages > 1 ? (
          ...
          >
            <TableHeader>
              <TableColumn allowsSorting key="last_hostname">
                Hostname
              </TableColumn>
              <TableColumn allowsSorting key="last_ip">
                IP Address
              </TableColumn>
              <TableColumn allowsSorting key="mac">
                MAC Address
              </TableColumn>
              <TableColumn allowsSorting key="last_vlan">
                VLAN
              </TableColumn>
              <TableColumn allowsSorting key="last_port_id">
                Port
              </TableColumn>
            ...
            <TableBody>
              {paginatedItems.map((client, index) => (
                <TableRow key={client.mac || index}>
                  <TableCell>{client.last_hostname || "-"}</TableCell>
                  <TableCell>{client.last_ip || "-"}</TableCell>
                  <TableCell>{formatMac(client.mac)}</TableCell>
                  <TableCell>{client.last_vlan || "-"}</TableCell>
                  <TableCell>{client.last_port_id || "-"}</TableCell>
          ...
          <div className="flex justify-center mt-2 text-gray-500 text-sm">
            ...
            {Math.min(page * ROWS_PER_PAGE, sortedAndFilteredClients.length)} of{" "}
